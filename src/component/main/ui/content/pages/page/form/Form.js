...
export default class Form extends Page {
  ...
  constructor(props){
    super(props);
    this.init(props);
    this.state = { submit: this.getSubmitByFormId(this.store.form.usingForm._id)  }
  }
    ...
    return (
      <View style={this.page()}>
        <ScrollView contentContainerStyle={this.style.list}>
          {this.gap(0.02)}
          {this.texts.general(definition.title)}
          {this.sep()}
          {definition.questions && definition.questions.map((q, i)=>{ return this.question(q); })}
          {this.gap(0.25)}
        </ScrollView>
      </View>
    )
  ...
  field(q){
    const value = this.state.submit[q.id].value;
    ...
    switch (q.type) {
      ...
      case 'slider':
        return this.inputs.slider([0.67, 0.075], value, q.minimum, q.maximum,
          (value)=>{ this.onChange(q.id, value); });
      case 'checkbox':
        return(
        <CheckBoxes app={this.app} choices={q.choices} value={value}
